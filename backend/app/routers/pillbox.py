...
def get_user_email(db: Session = Depends(get_db)):
    """Get stored user email (single-user demo)."""
...
def create_med(body: MedCreate, db: Session = Depends(get_db)):
    med = Med(
        name=body.name,
...
def list_meds(db: Session = Depends(get_db)):
    meds = db.query(Med).order_by(Med.created_at.desc()).all()
...
def get_med(med_id: int, db: Session = Depends(get_db)):
    med = db.query(Med).filter(Med.id == med_id).first()
    if not med:
...
def update_med(med_id: int, body: MedUpdate, db: Session = Depends(get_db)):
    med = db.query(Med).filter(Med.id == med_id).first()
    if not med:
        raise HTTPException(status_code=404, detail="Medication not found")
...
def create_schedule(med_id: int, body: ScheduleCreate, db: Session = Depends(get_db)):
    med = db.query(Med).filter(Med.id == med_id).first()
    if not med:
        raise HTTPException(status_code=404, detail="Medication not found")
    s = Schedule(
...
def list_schedules(med_id: int, db: Session = Depends(get_db)):
    med = db.query(Med).filter(Med.id == med_id).first()
    if not med:
        raise HTTPException(status_code=404, detail="Medication not found")
